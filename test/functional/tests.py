...
class TestObjectVersioning(Base):
    ...
    def test_versioning_dlo(self):
        ...
        # unless I prefix the obj name with MANIFEST_, sometimes
        # there's an error in the
        # self.assertEqual(man_file.info()['x_object_manifest'],
        # below caused by getting a 409 when writing the manifest file
        man_file = container.file('MANIFEST_' + obj_name)
        ...
        # write manifest file again with different header
        man_file.write('', hdrs={"X-Object-Manifest": "%s/%s_d/" %
                       (self.env.container.name, obj_name)})

        # check there are 4 files versioned (including manifest)
        self.assertEqual(4, versions_container.info()['object_count'])

        # delete manifest with different header and make sure versioned
        # manifest is copied back
        man_file.delete()
        man_file.initialize()
        self.assertEqual(3, versions_container.info()['object_count'])
        self.assertEqual(man_file.info()['x_object_manifest'],
                         '%s/%s/' % (self.env.container.name, obj_name))
