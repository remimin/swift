...
Add this middleware to the proxy pipeline after the authentication
middleware.

Set ``allow_versioned_writes`` to ``True`` in the middleware options
to enable information about this middleware to be returned in a /info
request.
...
If you want to disable all functionality, remove this middleware from
the proxy pipeline.
...
class VersionedWritesContext(WSGIContext):
    ...
    def handle_obj_versions_put(self, req, object_versions,
        ...
        # X-Versions-Location
        if is_success(hresp.status_int):
            ...
            # handle slo and dlo manifest versioning
            # add query string in case it is a manifest file
            # otherwise it just gets ignored
...
class VersionedWritesMiddleware(object):
    ...
    def container_request(self, req, start_response):
        ...
        if val:
            ...
            del req.headers['X-Versions-Location']
    ...
    def __call__(self, env, start_response):
        ...
        # the middleware becomes the authority on whether the object
        # versioning is enabled or not. In case it is not set, then
        # the option in the container configuration is still checked
        # for backwards compatibility
        if config_true_value(self.conf.get('allow_versioned_writes')) \
                and container and not obj:
