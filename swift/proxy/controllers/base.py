...
def headers_to_container_info(headers, status_int=HTTP_OK):
    ...
    return {
        ...
        'storage_policy': headers.get('X-Backend-Storage-Policy-Index'.lower(),
                                      '0'),
        'shardtrie': headers.get('x-backend-shardtrie', ''),
        'cors': {
...
class Controller(object):
    ...
    def get_shard_trie(self, req, account_name, container_name, marker=None,
        ...
        if end_marker:
            environ['QUERY_STRING'] += '&end_marker=%s' % quote(end_marker)
        if limit:
            try:
                environ['QUERY_STRING'] += '&limit=%d' % int(limit)
            except ValueError:
                # let swift use the default value.
                pass
        environ['REQUEST_METHOD'] = 'GET'
        environ['HTTP_ACCEPT'] = 'application/trie'
        environ.pop('PATH_INFO')
        new_req = Request.blank(path, environ)
